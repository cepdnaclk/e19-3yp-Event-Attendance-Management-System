        ...
        const fetchAttendeeData = async () => {
            try {
                ...
                const detailsPromises = dataIds.attendeeIds.map(async (id) => {
                    const responseDetails = await fetch(`http://localhost:5001/api/attendees/${id}/details`);
    ...
    useEffect(() => {
        const fetchRegAttendeeData = async () => {
            try{
                ...
                const sessionDetailsResponse = await axios.get(`http://localhost:5001/api/sessions/${sessionId}`);
